...
class _EditTaskPageState extends State<EditTaskPage> {
  ...
  void initState() {
    ...
    //get info from task
    edtSubCtrl.text = widget.task.subject;
    edtTextCtrl.text = widget.task.text;
    taskTypeSelected = Constants.taskTypeList().indexWhere((e) {
      return e.type == widget.task.taskType.type;
    });
  }
  ...
  void saveTask() async {
    //edit and save new task in database
    if ((edtSubCtrl.text.isNotEmpty || edtTextCtrl.text.isNotEmpty) &&
        (taskTypeSelected != -1)) {
      widget.task.subject = edtSubCtrl.text;
      widget.task.text = edtTextCtrl.text;
      widget.task.taskType = Constants.taskTypeList()[taskTypeSelected];
      widget.task.save();
      Navigator.pop(context);
    }
  }
